...
export const getScenarioResultCached = cache(async (key: string) => {
...
export const getScenarioResult = async (
  game: Game
): Promise<ScenerioResult | null> => {
  let key =
    game.players[game.hero].name + "_" + game.players[game.hero].cards[0];
  game.actions.map((action, index) => {
    if (game.actions[index].choice !== "ante") {
      key += "_" + game.actions[index].choice;
    }
  });
  console.log("getScenarioResult: key ", key);
  ...
  return await getScenarioResultCached(key);
};
